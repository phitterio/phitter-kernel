...
if __name__ == "__main__":
    ...
    print(f"adinf(z) = {adinf(z)}")
    print(f"errfix(n,x) = {errfix(n, adinf(z))}")
    print(f"Prob(A_{n} < {z}) = {AD(n, z)}")
    print(f"CriticalValue({confidence_level}, {n}) = {ad_critical_value(confidence_level, n)}")
    print(f"PValue({n}, {z}) = {ad_p_value(n, z)}")
